import { Product, Order } from '../types';

export type AdminTab = 'dashboard' | 'users' | 'vendors' | 'products' | 'orders' | 'payments' | 'reports' | 'settings' | 'deliveri' | 'support' | 'kyc' | 'withdrawals';

export interface AdminUser {
  id: string;
  name: string;
  email: string;
  role: 'buyer' | 'vendor' | 'admin';
  status: 'Active' | 'Suspended';
  phone: string;
  dateJoined: string;
  totalOrders: number;
  totalSpent: number;
}

export interface AdminVendor {
  id: string;
  name: string;
  email: string;
  phone: string;
  status: 'Approved' | 'Pending' | 'Rejected';
  totalProducts: number;
  totalSales: number;
  rating: number;
  joiningDate: string;
  storeCategory: string;
  // null/undefined = uses the platform's global commission rate.
  commissionPercent?: number | null;
  accountStatus?: 'Active' | 'Suspended' | 'Under Review';
  storeStatus?: 'Pending' | 'Active' | 'Suspended' | 'Rejected';
  payoutStatus?: 'Active' | 'On Hold' | 'Frozen';
  suspensionReason?: string | null;
}

export interface Transaction {
  id: string;
  buyerName: string;
  email: string;
  amount: number;
  status: 'success' | 'failed' | 'reversed' | 'pending';
  paymentMethod: 'Paystack' | 'Flutterwave' | 'Bank Transfer' | 'Card' | 'Wallet';
  date: string;
  reference: string;
}

export interface ForumReport {
  id: string;
  reporterName: string;
  subject: string;
  type: 'Product Dispute' | 'Failed Payout' | 'Seller Fraud' | 'Delivery Complaint';
  status: 'Open' | 'Resolved' | 'Investigating';
  date: string;
  description: string;
}

export interface AppSettings {
  appName: string;
  commissionPercent: number;
  paymentGateway: 'Paystack' | 'Flutterwave' | 'Dual';
  testMode: boolean;
  payoutFrequency: 'Daily' | 'Weekly' | 'Monthly';
  flatDeliveryFee: number;
  restrictedCategories: string[];
}
