...
import { 
  ...
  Mail, Phone, Calendar, ShoppingBag, DollarSign, Search, ChevronLeft, ChevronRight, Filter, Save
} from 'lucide-react';
...
interface AdminVendorsViewProps {
  ...
  onUpdateVendorCommission?: (vendorId: string, commissionPercent: number | null) => void;
  currencySymbol?: string;
  isDarkMode?: boolean;
}

// A small inline editor for a vendor's commission override. Kept as its
// own component so each row has independent input state without needing
// a Record<vendorId, string> living in the parent.
function CommissionCell({
  ...
  onSave: (vendorId: string, commissionPercent: number | null) => void;
}) {
  ...
  return (
    <div className="flex items-center justify-center gap-1">
      <input
        ...
        placeholder="Default"
        title={value == null ? 'Using the platform default rate' : `Custom rate: ${value}%`}
      ...
      {isDirty && (
        <button
          onClick={() => onSave(vendorId, draft.trim() === '' ? null : Number(draft))}
          title="Save commission rate"
          className="p-1 bg-emerald-500/15 text-emerald-600 dark:text-[#95C93D] hover:bg-emerald-500/25 rounded-lg border border-emerald-500/20 cursor-pointer"
        >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Review vendor applications, verify documents, and manage active vendor stores.
          </p>
        </div>
          ...
          <table className="w-full border-collapse text-left">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-950/50 border-b border-gray-100 dark:border-slate-800/80">
                ...
                <th className="py-3 px-5 text-xs font-black uppercase text-gray-400 tracking-wider text-center">Commission</th>
                    ...
                    <td className="py-3.5 px-5">
                      {onUpdateVendorCommission && (
                        <CommissionCell
                          ...
                          onSave={onUpdateVendorCommission}
                        />
                      )}
                    </td>
                ...
                <div className="grid grid-cols-2 gap-3.5 bg-slate-50 dark:bg-slate-950/80 p-4 border border-gray-150 dark:border-slate-850 rounded-xl">
                  ...
                  <div className="space-y-1">
                    ...
                    <span className={`font-bold flex items-center gap-1.5 ${
                      ...
                      <span>96% Reliability Rating</span>
                    </span>
                  </div>
                </div>
