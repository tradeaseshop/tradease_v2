...
export default function BuyerMode({
  ...
  const handleSendChatMessage = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (lower.includes('deliver') || lower.includes('shipping') || lower.includes('track') || lower.includes('dispatch') || lower.includes('charger')) {
        ...
        botResponse = "Our ebooks are protected with a watermark tied to your account, so please don't share your copy with others.";
  ...
  return (
    <div className="h-full flex flex-col justify-between bg-slate-50 dark:bg-slate-900 overflow-hidden relative">
      ...
      {/* Floating Customer Care / Live Chat Bot Activation Button styled like WhatsApp */}
      <button
        onClick={() => setIsChatOpen(true)}
        className="absolute bottom-24 right-4 z-40 w-12 h-12 bg-[#25D366] hover:bg-[#20ba5a] rounded-full flex items-center justify-center text-white shadow-xl hover:shadow-[#25D366]/20 active:scale-95 cursor-pointer hover:scale-105 duration-200 border border-white/20 relative group"
