...
export default function BuyerMode({
  ...
  const [chatLog, setChatLog] = useState<Array<{ sender: 'user' | 'bot' | 'system', text: string, time: string }>>([
    ...
    { sender: 'bot', text: 'Hello! Welcome to TradeEase Support. Ask me about tracking your order, payments, ebooks, or wallet withdrawals.', time: 'Just Now' }
  ]);
  ...
  const handleSendChatMessage = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (lower.includes('deliver') || lower.includes('shipping') || lower.includes('track') || lower.includes('dispatch') || lower.includes('charger')) {
        ...
        botResponse = "We hold your payment safely until you confirm your order has arrived, then release it to the seller. You can approve an order any time from your Orders tab.";
        ...
        botResponse = "Disputed orders have their payment placed on hold right away while our team reviews the case.";
      }
  ...
  const handleSelectAIStyle = async (selectedStyle: string) => {
    ...
    try {
      const resp = await fetch(`${api.API_BASE}/api/gemini/recommend`, {
        method: 'POST',
  ...
  const handleSendCopilotMessage = async (textToSend?: string) => {
    ...
    try {
      const resp = await fetch(`${api.API_BASE}/api/gemini/chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleCheckoutSubmit = async (e: React.FormEvent) => {
    ...
    if (paymentMethod === 'wallet') {
      if (walletBalance < totalWithFulfillment) {
        setCheckoutError(`Insufficient Funds: Your wallet balance is ${formatNaira(walletBalance)}, but the total bill (items + shipping) is ${formatNaira(totalWithFulfillment)}. Please fund your wallet under the Orders tab or switch to Direct Bank Transfer!`);
        return;
      }
        ...
        {isDrawerOpen && (
          <>
            ...
            >
              ...
              {/* Vendor Switching Button inside Drawer — only relevant while
                  browsing without an account. Once signed in with a specific
                  role, that account determines which UI is shown. */}
              {!currentUser && (
          ...
          <>
            ...
            >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <span className="text-[9px] text-gray-400 font-bold block font-mono">
                      Online • We reply within minutes
                    </span>
                  </div>
                </div>
                <button 
              ...
              <form onSubmit={handleSendChatMessage} className="flex gap-2 pt-2 border-t border-gray-100 dark:border-gray-850">
                <input
                  type="text"
                  placeholder="Ask about delivery, orders, returns..."
                  value={chatMessage}
