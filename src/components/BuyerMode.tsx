...
const BANNER_CAMPAIGNS = [
  ...
  {
    id: 3,
    title: "🛡️ Safe Escrow Release PROTECTION",
    tagline: "Your deposit is held securely. Confirmed payout triggers only when you inspect.",
    image: "https://images.unsplash.com/photo-1563013544-824ae1d704d3?auto=format&fit=crop&q=80&w=600",
    actionLabel: "How it works",
    color: "from-indigo-600 to-blue-700",
    badge: "ESCROW CLEARING"
  }
];
...
export default function BuyerMode({
  ...
  // Live Support Chat Bot States (Item 9)
  const [isChatOpen, setIsChatOpen] = useState(false);
  const [chatMessage, setChatMessage] = useState('');
  const [chatLog, setChatLog] = useState<Array<{ sender: 'user' | 'bot' | 'system', text: string, time: string }>>([
    { sender: 'system', text: 'TradeEase Automated Concierge Active', time: 'Just Now' },
    { sender: 'bot', text: 'Hello! Welcome to TradesEase Nigeria Support. Ask me anything about tracking your DELIVERI dispatch, escrow settlements, e-books encryption, or wallet withdrawals.', time: 'Just Now' }
  ]);
  ...
  const handleSendChatMessage = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      let botResponse = "I have logged your request. One of our human TradeEase backoffice administrators will review this shortly! You can also check on the Corporate Admin Terminal for central logs.";
      const lower = userText.toLowerCase();

      if (lower.includes('deliver') || lower.includes('shipping') || lower.includes('track') || lower.includes('dispatch') || lower.includes('charger')) {
        botResponse = "DELIVERI uses dynamic calculation based on distance, weight & urgency. You can track your orders by entering the DELIVERI Tracking number (e.g. DV-381-8172) in your active waybills.";
      } else if (lower.includes('escrow') || lower.includes('approve') || lower.includes('settle')) {
        botResponse = "TradeEase uses deep escrow safeguards. Funds are locked into escrow upon check-out, and are only dispatched to the seller when the buyer clicks 'Approve Order' inside the customer portal.";
      } else if (lower.includes('ebook') || lower.includes('book') || lower.includes('encryption')) {
        botResponse = "TradeEase E-Books are encrypted natively. When you purchase an e-book list item, you will be given a secure decryption license that restricts transfer or leaking to unregistered readers.";
      } else if (lower.includes('wallet') || lower.includes('payout') || lower.includes('withdrawal') || lower.includes('bank') || lower.includes('paystack')) {
        botResponse = "Our Wallet System supports instant Direct Bank Transfer or automated payout withdrawals. Buyers can top-up easily, and sellers can request swift settlements.";
      } else if (lower.includes('dispute') || lower.includes('complaint')) {
        botResponse = "Disputed orders lock respective escrow funds immediately. Safe arbitration is conducted by our backoffice team to guarantee fairness for both parties.";
      }

      setChatLog(prev => [...prev, { sender: 'bot', text: botResponse, time: timeStr }]);
    }, 750);
  };

  // ==========================================
  // TradeEase AI Personalization & Copilot States
  // ==========================================
  const [buyerPersona, setBuyerPersona] = useState<string>(() => {
    return localStorage.getItem('tradeease_ai_persona') || 'general';
  });
  const [isRecommending, setIsRecommending] = useState(false);
  const [aiRecommendations, setAiRecommendations] = useState<Array<{productId: string, personalizationReason: string}>>(() => {
    const saved = localStorage.getItem('tradeease_ai_recommendations');
    return saved ? JSON.parse(saved) : [];
  });
  const [aiAdvice, setAiAdvice] = useState<string>(() => {
    return localStorage.getItem('tradeease_ai_advice') || '';
  });
  const [aiPersonaName, setAiPersonaName] = useState<string>(() => {
    return localStorage.getItem('tradeease_ai_persona_name') || 'TradeEase AI Assistant';
  });
  const [aiCopilotOpen, setAiCopilotOpen] = useState(false);
  const [copilotInput, setCopilotInput] = useState('');
  const [isCopilotTyping, setIsCopilotTyping] = useState(false);
  const [copilotMessages, setCopilotMessages] = useState<Array<{role: 'user' | 'assistant', content: string}>>(() => {
    const saved = localStorage.getItem('tradeease_copilot_messages');
    return saved ? JSON.parse(saved) : [
      { role: 'assistant', content: "Hello! I am your TradeEase AI Shopping Copilot, powered by Gemini 3.5. ⚡\nI am fully connected with our live marketplace catalog. Tell me what you're looking for, or let me guide you to the perfect purchase!" }
    ];
  });

  // Persist AI configurations locally
  useEffect(() => {
    localStorage.setItem('tradeease_ai_persona', buyerPersona);
    localStorage.setItem('tradeease_ai_recommendations', JSON.stringify(aiRecommendations));
    localStorage.setItem('tradeease_ai_advice', aiAdvice);
    localStorage.setItem('tradeease_ai_persona_name', aiPersonaName);
  }, [buyerPersona, aiRecommendations, aiAdvice, aiPersonaName]);

  useEffect(() => {
    localStorage.setItem('tradeease_copilot_messages', JSON.stringify(copilotMessages));
  }, [copilotMessages]);

  const handleSelectAIStyle = async (selectedStyle: string) => {
    setBuyerPersona(selectedStyle);
    setIsRecommending(true);
    
    // Map style to appropriate interests
    let interests: string[] = [];
    let budget = 'any';
    if (selectedStyle === 'budget') {
      interests = ['ebooks', 'food', 'beauty'];
      budget = '25000';
    ...
    try {
      ...
      if (resp.ok) {
        ...
        if (data.aiPersonaName) setAiPersonaName(data.aiPersonaName);
      }
    } catch (err) {
    ...
    try {
      const resp = await fetch('/api/gemini/chat', {
        ...
        body: JSON.stringify({
          ...
          currentPreferences: { state: location.state, city: location.city },
          products
        })
      });
      if (resp.ok) {
      ...
      if (oldStatus !== undefined && oldStatus !== order.status) {
        ...
        if (!isFirstMountRef.current) {
          ...
          if (order.status === 'Shipped') {
            ...
            alertMsg = `✅ Order ${order.id} is confirmed Delivered. Escrow funds have been successfully settled to the vendor.`;
          } else if (order.status === 'Cancelled') {
            alertMsg = `⚠️ Waybill ${order.id} was Cancelled. Respective escrow deposits will return to your balance.`;
  ...
  const runEbookDecryptorSimulation = (productId: string, watermarkText?: string) => {
    ...
    const logs = [
      "Connecting to TradeEase sovereign digital license vault...",
      `Validating ledger clearance signature for licensee: [ ${buyerEmail} ]`,
      "Cleavage verification passed. Extracting cryptographic payload wrapper...",
      "Stripping file metadata container lock and obfuscation headers...",
      "Evaluating key seed verification. Running local fast-decryption in viewport...",
      `Applying dynamic anti-leak watermark layer: "${watermarkText || 'TradeEase Protection Licensed to ' + buyerEmail}"`,
      "File reconstructed! Local sandbox EPUB/PDF reader active."
    ];
  ...
  const [consentEscrowReminders, setConsentEscrowReminders] = useState(true);
  ...
  const handleCheckoutSubmit = async (e: React.FormEvent) => {
    ...
    const finalDeliveryCost = getDynamicDeliveriCost(activeProvider.baseFee);
    const totalWithFulfillment = cartTotalAmount + finalDeliveryCost;

    let paymentReference: string | undefined;
    let confirmedPaymentStatus: 'Pending' | 'Paid' = 'Pending';

    if (paymentMethod === 'wallet') {
      if (walletBalance < totalWithFulfillment) {
        setCheckoutError(`Insufficient Funds: Your escrow wallet balance is ${formatNaira(walletBalance)}, but the total bill (items + shipping) is ${formatNaira(totalWithFulfillment)}. Please fund your wallet under the Orders tab or switch to Direct Bank Transfer!`);
        return;
      }
  ...
  return (
    <div className="h-full flex flex-col justify-between bg-slate-50 dark:bg-slate-900 overflow-hidden relative">
      ...
      {/* Dynamic Real-time Escrow Status Toasts Feed */}
        ...
        {isDrawerOpen && (
          <>
            ...
            >
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between pb-4 border-b border-gray-100 dark:border-gray-800">
                  <div className="flex flex-col items-start gap-1">
                    <Logo size="sm" showTagline={false} animate={false} />
                    <span className="text-[9px] text-emerald-600 dark:text-emerald-400 font-bold uppercase tracking-wider pl-1 font-mono">
                      ● Buyer Console Enabled
                    </span>
                  </div>
                  <button 
                    onClick={() => setIsDrawerOpen(false)}
              ...
              {/* Vendor Switching Button inside Drawer */}
        ...
        <AnimatePresence mode="wait">
          ...
          {navTab === 'home' && (
            ...
            >
              ...
              {/* === AI PERSONALIZED EXPERIENCE LOUNGE === */}
              <div className="mx-4 bg-gradient-to-br from-indigo-950 to-slate-900 border border-indigo-500/30 text-white rounded-3xl p-4 space-y-3 shadow-lg relative overflow-hidden">
                {/* Decorative background flare */}
                <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-500/15 rounded-full blur-2xl pointer-events-none" />
                <div className="absolute -bottom-10 -left-10 w-24 h-24 bg-purple-500/15 rounded-full blur-2xl pointer-events-none" />
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h3 className="text-xs font-black uppercase tracking-wider text-amber-300">
                        TradeEase AI Experience Hub
                      </h3>
                      <p className="text-[9px] text-indigo-200 uppercase font-bold tracking-widest leading-none">
                        Powered by Gemini 3.5 Flash
                      </p>
                    </div>
                  </div>
                  <span className="text-[8px] bg-white/10 px-2 py-0.5 rounded-full uppercase tracking-wider font-extrabold text-indigo-300">
                    Active Persona: {buyerPersona === 'general' ? 'Core Feed' : buyerPersona.toUpperCase()}
                  </span>
                </div>

                <p className="text-[10px] text-slate-300 leading-snug">
                  Choose your shopping style. Our AI will curate special recommendations, adjust category order, and suggest financial budgets dynamically.
                </p>

                {/* Persona choice pills */}
                <div className="grid grid-cols-4 gap-1.5 pt-1 relative z-10">
                  {[
                    { id: 'budget', label: '🇳🇬 Budget', icon: '🪙', desc: 'Under ₦25k' },
                    { id: 'tech', label: '⚡ Tech', icon: '🔌', desc: 'Solar / Gear' },
                    { id: 'fashion', label: '👗 Style', icon: '👑', desc: 'Handwoven' },
                    { id: 'foodie', label: '🥘 Foodie', icon: '🍲', desc: 'Raw Foods' }
                  ].map((style) => (
                    <button
                      key={style.id}
                      onClick={() => handleSelectAIStyle(style.id)}
                      className={`p-2 rounded-xl text-center transition-all active:scale-95 flex flex-col items-center justify-center cursor-pointer border ${
                        buyerPersona === style.id
                          ? 'bg-amber-500 border-amber-400 text-slate-950 shadow-md font-extrabold scale-102'
                          : 'bg-white/5 border-white/10 text-white hover:bg-white/10'
                      }`}
                    >
                      <span className="text-xs">{style.icon}</span>
                      <span className="text-[8.5px] font-black tracking-tight block mt-0.5 leading-none">{style.label}</span>
                      <span className="text-[6.5px] opacity-70 block leading-tight mt-0.5">{style.desc}</span>
                    </button>
                  ))}
                </div>

                {/* Recommendations Carousel Area */}
                {isRecommending ? (
                  <div className="py-8 text-center space-y-2 animate-pulse">
                    <div className="w-8 h-8 rounded-full border-2 border-amber-400 border-t-transparent animate-spin mx-auto animate-duration-1000" />
                    <p className="text-[9px] text-gray-400 font-mono tracking-widest uppercase">TradeEase AI is compiling inventory...</p>
                  </div>
                ) : aiRecommendations.length > 0 ? (
                  <div className="space-y-3 pt-2 border-t border-white/10">
                    <div className="flex items-center gap-1 text-amber-300">
                      <Sparkles className="w-3.5 h-3.5 stroke-[2.5]" />
                      <h4 className="text-[10px] font-extrabold uppercase tracking-wider animate-pulse">
                        Curations by {aiPersonaName}
                      </h4>
                    </div>
                    ...
                    <div className="flex gap-3 overflow-x-auto pb-1 scrollbar-none">
                      {aiRecommendations.map((rec) => {
                        const targetProd = products.find(p => p.id === rec.productId);
                        if (!targetProd) return null;
                        return (
                          <div
                            key={rec.productId}
                            onClick={() => setSelectedProduct(targetProd)}
                            className="bg-white/5 border border-white/10 rounded-2xl p-2.5 w-[220px] shrink-0 text-left space-y-2 cursor-pointer hover:bg-white/10 transition-colors"
                          >
                            <div className="flex gap-2">
                              {/* Image thumbnail */}
                              <div className="w-12 h-12 rounded-xl overflow-hidden shrink-0 border border-white/15">
                                <img src={targetProd.image} alt={targetProd.title} className="w-full h-full object-cover" />
                              </div>
                              <div className="min-w-0 flex-1">
                                <h5 className="text-[10px] font-bold text-white truncate">{targetProd.title}</h5>
                                <p className="text-[10px] text-amber-300 font-black">₦{targetProd.price.toLocaleString()}</p>
                                <span className="inline-block text-[6.5px] bg-emerald-500/20 text-emerald-300 font-extrabold scale-90 -translate-x-1.5 px-1 py-0.5 rounded font-mono">
                                  98% PERSONAL MATCH
                                </span>
                              </div>
                            </div>
                            <p className="text-[8.5px] text-gray-300 leading-snug italic line-clamp-2">
                    ...
                    {/* Professional Advisor Advice Block */}
                    {aiAdvice && (
                      <div className="bg-amber-500/15 border border-amber-500/30 rounded-2xl p-3 text-left">
                        <span className="text-[7.5px] text-amber-300 font-bold tracking-widest uppercase block mb-1">
                          Personal Shopping Strategic Advice:
                        </span>
                        <p className="text-[9px] text-indigo-100 font-medium leading-relaxed">
                          {aiAdvice}
                        </p>
                      </div>
                    )}
                  ...
                  <div className="bg-white/5 rounded-2xl py-3 border border-white/5 text-center px-4">
                    <p className="text-[9px] text-indigo-200">
                      💡 Tip: Click one of the tags above. AI will scan our catalog and dynamically render curated listings best matching your target lifestyle profile.
                    </p>
                  </div>
                )}
              </div>
                  ...
                  {FEATURED_VENDORS.map((v) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-0.5 mt-1 bg-amber-500/10 px-1 py-0.5 rounded text-[8px] font-black text-amber-600">
                          <Star className="w-2 h-2 fill-amber-500 text-amber-500 stroke-0" />
                          <span>{v.rating}</span>
                        </div>
                      </button>
                    );
              ...
              <div className="space-y-3 px-4">
                ...
                <div className="mt-6 bg-gradient-to-br from-emerald-600/5 to-teal-600/5 dark:from-emerald-950/20 dark:to-teal-950/15 border border-emerald-500/10 rounded-2xl p-4 text-left space-y-2.5">
                  ...
                  <p className="text-[10.5px] text-slate-500 dark:text-slate-400 leading-relaxed font-semibold">
                    TradeEase is built as a sovereign bridge connecting local farmers, distributors, and buyers directly across Nigeria. By implementing durable, decentralized escrow safeguards and integrating with DELIVERI's automated distance-mass-urgency courier fleet, we prevent logistical delays and ensure direct payout security.
                  </p>
              ...
              <div className="bg-gradient-to-br from-emerald-950 to-slate-925 p-6 rounded-2xl text-white shadow-xl relative overflow-hidden border border-emerald-500/10">
                ...
                <div className="relative z-10 space-y-3">
                  ...
                  <p className="text-xs text-gray-250 leading-relaxed font-semibold">
                    TradeEase is a modern digital marketplace built to simplify how people buy and sell—bringing convenience, flexibility, and trust into one seamless platform.
                  </p>
                  <p className="text-[11px] text-gray-400 leading-relaxed font-medium">
                    We understand that today’s commerce is dynamic. Sometimes you’re a buyer. Other times, you’re a seller. That’s why TradeEase is designed as a dual-role platform, allowing users to switch effortlessly between buying and selling within a single app—no need for multiple accounts or complicated setups.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="p-5 bg-white dark:bg-slate-900 border border-gray-150 dark:border-slate-850 rounded-2xl shadow-xs relative overflow-hidden group">
                  ...
                  <div className="relative z-10 space-y-2">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">
                      To empower individuals and businesses by making online trading simple, accessible, and efficient for everyone.
                    </p>
                  </div>
                </div>
                ...
                <div className="p-5 bg-white dark:bg-slate-900 border border-gray-150 dark:border-slate-850 rounded-2xl shadow-xs relative overflow-hidden group">
                  ...
                  <div className="relative z-10 space-y-2">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">
                      To become a leading marketplace platform where anyone can connect, trade, and grow without barriers.
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 gap-3">
                  {[
                    ...
                    },
                    {
                      ...
                      text: "We provide tools for vendors to showcase products, manage listings, and grow their businesses without technical complexity."
                    },
                    {
      ...
      {/* Logistics Companies & Carrier Selector */}
                      ...
                      <div className="pt-3 pb-1 space-y-2 border-t border-gray-100 dark:border-gray-800">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          ...
                          >
                            ...
                            <span className="text-[9px] font-black uppercase tracking-tight">Escrow Wallet</span>
                          </button>
                      ...
                      {paymentMethod === 'bank' && (
                        <div className="p-3 bg-amber-50/50 dark:bg-amber-950/10 border border-amber-500/20 rounded-xl space-y-1.5 text-left">
                          ...
                          <div className="space-y-0.5 font-mono text-[10px] text-gray-600 dark:text-gray-300">
                            ...
                            <p>Account Name: <strong className="text-gray-900 dark:text-white">TradeEase Nigeria Escrow Ltd</strong></p>
                  ...
                  <div className="pt-4 border-t border-gray-100 dark:border-gray-800 shrink-0 space-y-2.5">
                    <div className="p-3 bg-slate-100 dark:bg-slate-950/60 rounded-xl space-y-1.5 text-xs text-gray-650 dark:text-gray-400 border border-gray-100 dark:border-white/5">
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-gray-500">Cart Cargo Items:</span>
                      ...
                      <div className="flex flex-col space-y-0.5 pt-0.5 border-b border-gray-250/20 pb-1.5">
                        <div className="flex items-center justify-between">
                          <span className="font-medium text-gray-500">Carrier Logistics Surcharge:</span>
                        ...
                        <div className="flex items-center justify-between text-[9px] text-gray-450 pl-2">
                          <span>• Payload Weight Surcharge ({cart.reduce((sum, item) => sum + item.quantity * getProductEstimatedWeight(item.product.category, item.product.title), 0).toFixed(1)} KG):</span>
                        ...
                        <div className="flex items-center justify-between text-[9px] text-gray-450 pl-2">
                          <span>• Distance Surcharge ({getBaseStateDistanceKm(location.state) + lastMileKm} KM total):</span>
                      ...
                      <div className="pt-1.5 flex items-center justify-between font-black text-gray-900 dark:text-white text-sm">
                        <span>Total Escrow Settlement:</span>
            ...
            >
                              ...
                              {isCurrentlyExpanded && (
                                ...
                                >
                                  ...
                                  <div className="space-y-3 pl-1.5 relative border-l-2 border-dashed border-gray-200 dark:border-gray-800 ml-2">
                                    ...
                                    {/* Step 2: Paystack / Flutterwave Escrow clearance */}
                                    <div className="relative pl-5 pb-0.5">
                                      <div className="absolute -left-[7px] top-0.5 w-3 h-3 rounded-full bg-emerald-500 border-2 border-white dark:border-slate-950 shadow-xs flex items-center justify-center" />
                                      <div className="leading-tight">
                                        <span className="text-[10.5px] font-black text-gray-900 dark:text-white flex items-center gap-1">
                                          <span>Secured Funds Placed in Escrow</span>
                                          <Check className="w-3 h-3 text-emerald-500 stroke-[4]" />
                                        </span>
                                    ...
                                    {/* Step 5: Delivered Escrow Released */}
                                    <div className="relative pl-5">
                                      ...
                                      <div className="leading-tight">
                                        <span className="text-[10.5px] font-black text-gray-900 dark:text-white">
                                          Delivered & Escrow Funds Released
                                        </span>
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-gradient-to-br from-emerald-650 to-teal-800 text-white rounded-2xl shadow-sm relative overflow-hidden flex flex-col justify-between min-h-[140px]">
                    ...
                    <div className="z-10 mt-3">
                      <span className="text-[10px] text-emerald-100 block opacity-80">Available Escrow Ledger Balance</span>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="bg-white dark:bg-slate-950 border border-gray-150 dark:border-gray-800/80 rounded-2xl divide-y divide-gray-100 dark:divide-gray-900 overflow-hidden text-xs">
                      <div className="p-3 flex items-center justify-between">
                        <div>
                          <p className="font-extrabold text-gray-900 dark:text-white">Active System Credit setup</p>
                          <span className="text-[9px] text-gray-400">Automatic demo allocations initialized</span>
                        </div>
                        <span className="font-bold text-emerald-600">+₦75,000</span>
                      </div>
                      <div className="p-3 flex items-center justify-between opacity-75">
                        <div>
                          <p className="font-extrabold text-gray-900 dark:text-white">Inbound Escrow Release Setup</p>
                          <span className="text-[9px] text-gray-400">TradeEase protective buffer online</span>
                        </div>
                        <span className="font-mono text-gray-400">Security Gate</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {profileSubTab === 'edit-profile' && (
                <div className="bg-white dark:bg-slate-950 border border-gray-150 dark:border-gray-800 rounded-2xl p-4 space-y-4">
                  <h4 className="text-xs font-black text-gray-900 dark:text-white uppercase tracking-wider">
                    Edit Personal Buyer Profile credentials
                  </h4>

                  <div className="space-y-3 text-xs">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-gray-400 uppercase">Contact Display Name</label>
                      <input
                        type="text"
                        value={profileName}
                        onChange={(e) => setProfileName(e.target.value)}
                        placeholder="Oluwaseun Adepoju"
                        className="w-full p-2.5 rounded-xl border border-gray-250 dark:border-gray-800 bg-gray-50/50 dark:bg-slate-950 focus:ring-1 focus:ring-emerald-500 focus:outline-hidden font-semibold"
                      />
                    </div>

                    <div className="space-y-1 font-mono">
                      <label className="text-[10px] font-bold text-gray-400 uppercase font-sans">SMS / WhatsApp Phone Number</label>
                      <input
                        type="text"
                        value={profilePhone}
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-gray-400 uppercase">Primary Email Address</label>
                      <input
                        type="email"
                        value={profileEmail}
                        onChange={(e) => setProfileEmail(e.target.value)}
                        placeholder="buyer@tradeease.ng"
                        className="w-full p-2.5 rounded-xl border border-gray-250 dark:border-gray-800 bg-gray-50/50 dark:bg-slate-950 focus:ring-1 focus:ring-emerald-500 focus:outline-hidden font-semibold"
                      />
                    </div>
                    ...
                    <button
                      onClick={() => {
                        // Persist edits inside custom local profiles
                        alert('Personal Profile Updates preserved locally on TradeEase device state!');
                        setProfileSubTab('info');
                      }}
                      className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-black uppercase tracking-wider cursor-pointer shadow-xs mt-2"
                    >
                      Apply and Update Profile
                    </button>
                  </div>
                </div>
              )}
              ...
              {profileSubTab === 'settings' && (
                <div className="space-y-4">
                  <div className="bg-white dark:bg-slate-950 border border-gray-150 dark:border-gray-800 rounded-2xl p-4 space-y-4">
                    ...
                    {/* Escrow safety alert switch */}
                    <div className="flex items-center justify-between text-xs pb-1">
                      <div>
                        <p className="font-extrabold text-gray-900 dark:text-white">Smart Escrow Payout Warnings</p>
                        <span className="text-[9px] text-gray-400 block max-w-[210px]">
                      ...
                      <input
                        type="checkbox"
                        checked={consentEscrowReminders}
                        onChange={(e) => setConsentEscrowReminders(e.target.checked)}
                        className="w-4 h-4 text-emerald-600 bg-gray-100 border-gray-300 rounded focus:ring-emerald-500 cursor-pointer"
                      />
                    </div>
                  </div>
                    ...
                    if (purchasedEbooks.length === 0) {
                      return (
                        <div className="text-center py-10 bg-white dark:bg-slate-950 rounded-2xl border border-dashed border-gray-200 dark:border-gray-800">
                          ...
                          <p className="text-xs text-gray-400">No active digital purchases found in the escrow ledger.</p>
                          <button
                            onClick={() => {
                            ...
                            <div className="flex items-start justify-between pb-2 border-b border-gray-100 dark:border-gray-900">
                              <div>
                                ...
                                <p className="text-[9px] text-gray-400 font-mono mt-0.5 leading-none">
                                  Licensed PDF/EPUB Offline Sandbox Reader • By {selectedEbookProduct.vendorName}
                                </p>
                              </div>
                            ...
                            <div className="border border-gray-250 dark:border-gray-850 rounded-xl p-4 bg-gray-550/5 dark:bg-slate-900 relative overflow-hidden select-none text-left">
                              ...
                              <div className="relative z-10 space-y-3 font-sans text-xs text-slate-750 dark:text-gray-300 leading-relaxed max-h-72 overflow-y-auto">
                                <span className="text-[9px] bg-purple-600 text-white px-1.5 py-0.5 rounded tracking-wider uppercase font-extrabold font-mono">
                                  Obfuscated Text Payload Decoded Live
                                </span>
                                
                                <div className="space-y-2 text-left">
                                  <h5 className="font-extrabold text-xs text-gray-950 dark:text-white uppercase">CHAPTER 1: THE SOVEREIGN BLACK-MARKET Blueprints</h5>
                                  <p className="text-slate-650 dark:text-gray-350 italic">
                                    "When operating a multivendor brand across Nigerian transport networks, the single greatest friction is not fuel surcharges, but rather clearing delays at state borders (known locally as Omonile tax centers or union tolls)..."
                                  </p>
                                  <p>
                                    To bypass standard Lagos-to-Ibadan trunk failures, always secure a pre-registered custom transit manifest via the sister networks defined in the Digital Directory. Direct escrow transfers ensure high clearance milestones are met before physical delivery triggers.
                                  </p>
                                  <h5 className="font-extrabold text-xs text-gray-950 dark:text-white uppercase mt-4">CHAPTER 2: FLUTTERWAVE & PAYSTACK SECURE INTEGRITY KEYS</h5>
                                  <p>
                                    Do not store secret private hooks directly on the browser viewports. This leads to simple client-side credential extraction. Instead, implement server-side reverse relays that perform authorization locks outside user inspection streams.
                                  </p>
                                  <h5 className="font-extrabold text-xs text-gray-950 dark:text-white uppercase mt-4">CHAPTER 3: LAND REGISTRY & CERTIFICATES OF OCCUPANCY</h5>
                                  <p>
                                    Verify land entries in Alausa using the TradeEase legal network API. Always check governor's consents, survey histories, and dynamic litigation states before lodging secondary milestone payments in escrow.
                                  </p>
                                </div>
                              </div>
                            </div>
              ...
              {profileSubTab === 'orders' && (
                <div className="space-y-4 animate-fadeIn">
                  <div className="flex items-center justify-between pb-1">
                    <div className="flex items-center gap-1.5 flex-1 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[9px] text-gray-400 block font-medium">Safe Escrow Waybills</p>
                      </div>
                    </div>
                  ...
                  {(() => {
                    ...
                    const escrowLocked = orders.filter(o => o.status === 'Pending' || o.status === 'Processing' || o.status === 'Shipped')
                                ...
                                <div className="border-t border-gray-100 dark:border-gray-900/50 p-3.5 bg-slate-550/5 dark:bg-slate-900/10 space-y-3.5 text-left">
                                  ...
                                  {(() => {
                                    ...
                                    if (od.status === 'Delivered') {
                                      ...
                                      statusSubtext = "Recipient confirmed physical inspection. Escrow payout funds have been released to the vendor's digital ledger.";
                                      ...
                                      statusSubtext = "This waybill transaction was cancelled. Locked escrow deposit holds will return to your primary consumer balance.";
                                      ...
                                      statusSubtext = "Consignment is active on interstate dispatch motor network. Escrow remains guarded in safe-vault.";
                                  ...
                                  <div className="bg-white dark:bg-slate-950 border border-gray-150 dark:border-gray-800 p-3 rounded-xl space-y-2">
                                    ...
                                    <div className="pt-2 border-t border-gray-100 dark:border-slate-900 space-y-1 text-[10.5px] font-semibold text-gray-500 dark:text-gray-400 font-sans">
                                      ...
                                      <div className="flex justify-between text-xs font-black text-emerald-650 dark:text-emerald-400 pt-1 border-t border-dashed border-gray-150 dark:border-gray-800">
                                        <span>Escrow Guarded Total:</span>
                                  ...
                                  <div className="bg-white dark:bg-slate-950 border border-gray-150 dark:border-gray-800 p-3.5 rounded-xl space-y-3">
                                    ...
                                    <div className="space-y-3.5 pl-1.5 relative border-l border-dashed border-slate-200 dark:border-slate-800 ml-1 pb-1">
                                      ...
                                      <div className="relative pl-5">
                                        ...
                                        <div className="text-left leading-normal">
                                          <p className="text-[10px] font-black text-gray-900 dark:text-white flex items-center gap-1.5">
                                            <span>Order Placed & Certified</span>
                                            <Check className="w-3 h-3 text-emerald-500 stroke-[4]" />
                                          </p>
                                      ...
                                      {/* Point 2: Paystack / Flutterwave Escrow clearance */}
                                      <div className="relative pl-5">
                                        <div className="absolute -left-[4.5px] top-1 w-2 h-2 rounded-full bg-emerald-500" />
                                        <div className="text-left leading-normal">
                                          <p className="text-[10px] font-black text-gray-900 dark:text-white flex items-center gap-1.5">
                                            <span>Secured Escrow Wallet Deposit Lock</span>
                                            <Check className="w-3 h-3 text-emerald-500 stroke-[4]" />
                                          </p>
                                          <span className="text-[8.5px] text-gray-400 dark:text-gray-500 block">Funds cleared and locked in secure escrow ledger.</span>
                                        </div>
                                      </div>
                                      ...
                                      {/* Point 5: Delivered Escrow Released */}
                                      <div className="relative pl-5">
                                        <div className={`absolute -left-[4.5px] top-1 w-2 h-2 rounded-full ${
                                        ...
                                        <div className="text-left leading-normal">
                                          <p className="text-[10px] font-black text-gray-900 dark:text-white">
                                            Destination Arrival & Escrow Payout
                                          </p>
                                          <span className="text-[8.5px] text-gray-400 dark:text-gray-500 block">Buyer completes offline physical verification & approves Escrow release.</span>
                                        </div>
                                      </div>
                                  ...
                                  <div className="flex flex-col gap-2 pt-1 border-t border-gray-100 dark:border-slate-900">
                                    <div className="flex gap-2">
                                      ...
                                      <button
                                        onClick={() => {
                                          setActiveDisputeMessage(`Opened dispute file #DSP-${od.id.slice(1)}. TradeEase sovereign dispute handlers have locked these funds in physical clearing escrow. Our Lagos center is contacting vendor: ${od.items[0]?.productTitle ? 'For Item: ' + od.items[0].productTitle : 'System'}.`);
                                        }}
              ...
              {/* UTILITY FOOTER ACTIONS (switching role, etc.) */}
              {profileSubTab === 'info' && (
                <div className="space-y-2 pt-1 select-none">
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span>Sell and list products as Nigerian Vendor</span>
                    </div>
                    <span>→</span>
                  </button>
                </div>
                ...
                <div className="space-y-4 pt-1">
                  ...
                  <div className="p-3 bg-slate-950/30 rounded-xl text-[9.5px] text-gray-400 space-y-1 leading-normal">
                    <div className="flex items-center gap-1.5 text-amber-500 font-bold uppercase tracking-wider">
                      ...
                      <span>Security Standard Guard (CBN Regulations)</span>
                    </div>
                    <p>
                      Payments are escrowed securely at TradeEase hubs and only finalized after delivery is verified. We protect both consumers and local merchants.
                    </p>
                  </div>
                ...
                <div className="py-6 space-y-4 text-center">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-[11px] text-gray-400 leading-relaxed max-w-xs mx-auto">
                      Splendid! Your wallet was credited with <span className="font-bold text-emerald-400">{formatNaira(parseFloat(fundingAmount) || 0)}</span>. Secure escrow is ready to settle with network vendors.
                    </p>
                  </div>
                  ...
                  <div className="bg-slate-950/60 p-3 rounded-xl border border-white/5 space-y-1.5 text-left text-xs max-w-xs mx-auto">
                    <div className="flex justify-between font-bold">
                      <span className="text-gray-400">Total Escrow Balance:</span>
      ...
      {/* Floating Customer Care / Live Chat Bot Activation Button styled like WhatsApp (Item 9) */}
      <button
        onClick={() => setIsChatOpen(true)}
        className="absolute bottom-20 right-[74px] z-40 w-12 h-12 bg-[#25D366] hover:bg-[#20ba5a] rounded-full flex items-center justify-center text-white shadow-xl hover:shadow-[#25D366]/20 active:scale-95 cursor-pointer hover:scale-105 duration-200 border border-white/20 relative group"
      ...
      >
        ...
        <div className="relative flex items-center justify-center">
          ...
          <span className="absolute -top-1.5 -right-1.5 w-2.5 h-2.5 bg-amber-500 rounded-full border border-white dark:border-slate-900" />
          <MessageCircle className="w-5 h-5 text-white stroke-[2.5]" />
        </div>
        <span className="absolute bottom-14 left-1/2 -translate-x-1/2 bg-slate-950 border border-white/10 text-[#25D366] font-black text-[8.5px] px-2.5 py-1 rounded-lg whitespace-nowrap shadow-md opacity-0 group-hover:opacity-100 transition-opacity uppercase tracking-wider pointer-events-none">
          💬 Care Support
        </span>
      </button>

      {/* Live Support Customer Care Chat Bot Drawer Overlay (Item 9) */}
      <AnimatePresence>
        {isChatOpen && (
          <>
            <motion.div
            ...
            <motion.div
              ...
              exit={{ y: '100%' }}
              transition={{ type: "spring", damping: 28, stiffness: 220 }}
              className="absolute bottom-0 left-0 right-0 h-[82%] bg-white dark:bg-slate-950 rounded-t-3xl z-40 p-4 flex flex-col text-left"
            >
              {/* Header */}
              ...
              {/* Chat Log Body */}
              <div className="flex-1 overflow-y-auto p-1.5 space-y-3 my-2 scrollbar-thin">
                {chatLog.map((log, idx) => (
                  <div 
                    key={idx} 
                    className={`flex flex-col ${
                      log.sender === 'user' ? 'items-end' : log.sender === 'system' ? 'items-center' : 'items-start'
                    }`}
                  >
                    {log.sender === 'system' ? (
                      <span className="text-[8.5px] bg-slate-100 dark:bg-slate-900 text-slate-500 px-2 py-0.5 rounded font-mono font-extrabold">
                        {log.text}
                      </span>
                    ) : (
                      <div className="max-w-[85%] space-y-0.5">
                        <span className="text-[8px] text-gray-450 block font-bold px-1">
                          {log.sender === 'user' ? 'You' : 'Care Agent'} • {log.time}
                        </span>
                        <div className={`p-2.5 rounded-2xl text-xs leading-normal ${
                          log.sender === 'user' 
                            ? 'bg-emerald-600 text-white rounded-tr-none font-medium' 
                            : 'bg-slate-105 dark:bg-slate-900 text-gray-850 dark:text-gray-200 rounded-tl-none border border-slate-150 dark:border-slate-800'
                        }`}>
                          {log.text}
                        </div>
                      </div>
              ...
              {/* Input Footer Form */}
              <form onSubmit={handleSendChatMessage} className="flex gap-2 pt-2 border-t border-gray-100 dark:border-gray-850">
                <input
                  type="text"
                  placeholder="Ask about delivery fees, disputes, escrow, ebooks..."
                  value={chatMessage}
                  onChange={(e) => setChatMessage(e.target.value)}
                  className="flex-1 bg-slate-50 dark:bg-slate-900 border border-gray-200 dark:border-gray-800 rounded-xl px-3 py-2 text-xs focus:ring-1 focus:ring-amber-500 dark:text-white"
                />
                <button
                  type="submit"
                  className="px-3 bg-emerald-600 hover:bg-emerald-500 font-extrabold text-white text-xs rounded-xl uppercase tracking-wider cursor-pointer"
                >
                  Send
                </button>
              </form>
      ...
      {/* 8. GLOWING AI COPILOT FLOATING BUTTON */}
      <div className="absolute bottom-20 right-4 z-40">
        <motion.button
          onClick={() => setAiCopilotOpen(true)}
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          className="w-12 h-12 bg-gradient-to-tr from-amber-500 to-indigo-600 rounded-full flex items-center justify-center text-white shadow-xl hover:shadow-amber-500/20 cursor-pointer border border-white/20 relative group"
        >
          <div className="absolute inset-0 rounded-full bg-amber-400 animate-ping opacity-20 group-hover:opacity-40" />
          <Sparkles className="w-5 h-5 text-white animate-pulse" />
          <span className="absolute bottom-14 left-1/2 -translate-x-1/2 bg-slate-950 border border-white/10 text-amber-300 font-extrabold text-[8.5px] px-2.5 py-1 rounded-lg whitespace-nowrap shadow-md opacity-0 group-hover:opacity-100 transition-opacity uppercase tracking-wider pointer-events-none">
            ⚡ AI Copilot
          </span>
        </motion.button>
      </div>

      {/* 9. AI COPILOT DRAMATIC SLIDE-UP DRAWER PANEL */}
      <AnimatePresence>
        {aiCopilotOpen && (
          <>
            {/* Backdrop */}
            <motion.div
              ...
              className="absolute inset-0 bg-slate-950/80 backdrop-blur-xs z-50 pointer-events-auto"
            />
            ...
            <motion.div
              ...
              exit={{ y: '100%' }}
              transition={{ type: "spring", damping: 28, stiffness: 220 }}
              className="absolute bottom-0 left-0 right-0 h-[85%] bg-slate-900 border-t border-amber-500/20 rounded-t-3xl z-55 flex flex-col pointer-events-auto overflow-hidden text-white"
            >
              {/* Decorative border accent */}
              <div className="h-1 bg-gradient-to-r from-amber-500 via-indigo-500 to-emerald-500 w-full" />
              
              {/* Drawer Header */}
              <div className="p-4 bg-slate-950 border-b border-white/5 flex items-center justify-between text-left">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className="text-[7.5px] bg-amber-500/20 text-amber-300 font-extrabold px-1.5 py-0.5 rounded font-mono uppercase tracking-widest block w-max">
                      Gemini-3.5 Realtime
                    </span>
                    <h3 className="font-extrabold text-xs text-white">TradeEase AI Copilot</h3>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    onClick={() => setCopilotMessages([
                      { role: 'assistant', content: "Muted past interactions. Let's restart our shopping exploration. What are we listing or buying in Nigeria today?" }
                    ])}
                    className="px-2 py-1 rounded bg-white/5 hover:bg-white/10 text-gray-400 text-[8.5px] font-bold uppercase cursor-pointer"
                  ...
                  <button
                    onClick={() => setAiCopilotOpen(false)}
                    className="p-1.5 rounded-full bg-white/5 hover:bg-white/10 text-gray-400 cursor-pointer"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin flex flex-col">
                ...
                {/* Shopping Prompt suggestions (shows only when message count is low) */}
                {copilotMessages.length <= 1 && (
                  <div className="space-y-2 mb-2 text-left shrink-0">
                    <span className="text-[7.5px] text-gray-500 font-black uppercase tracking-widest block">
                      Tap to ask your personalized TradeEase advisor:
                    </span>
                    <div className="grid grid-cols-1 gap-1.5 text-xs">
                      {[
                        { text: "🎁 Suggest top premium fashion and books for a wedding gift under ₦45,000", key: "gift" },
                        { text: "🍛 Give me an authentic Nigerian recipe that uses Elubo (Yam Flour)", key: "yam" },
                        { text: "🔌 Recommend standing fans or power backup accessories for hot afternoons", key: "fan" }
                       ].map((suggest, i) => (
                        <button
                          key={i}
                          onClick={() => handleSendCopilotMessage(suggest.text)}
                          className="p-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 text-left text-[10px] text-indigo-200 transition-all duration-200 cursor-pointer active:scale-98"
                        >
                          {suggest.text}
                        </button>
                      ))}
                ...
                <div className="space-y-4 flex-1 text-left">
                  {copilotMessages.map((msg, idx) => (
                    ...
                    >
                      <span className="text-[7.5px] text-gray-405 font-bold px-1 mb-1">
                        {msg.role === 'user' ? 'You' : 'TradeEase AI Advisor'}
                      </span>
                      <div
                        className={`p-3 rounded-2xl text-[11px] leading-relaxed max-w-[88%] whitespace-pre-wrap shadow-sm ${
                          msg.role === 'user'
                            ? 'bg-amber-500 text-slate-950 rounded-tr-none font-bold'
                            : 'bg-white/5 border border-white/10 text-gray-200 rounded-tl-none font-medium'
                        }`}
                      >
                        {msg.content}
                      </div>
                  ...
                  {isCopilotTyping && (
                    <div className="flex flex-col items-start">
                      <span className="text-[7.5px] text-gray-500 font-bold px-1 mb-1">TradeEase AI</span>
              ...
              >
                <input
                  type="text"
                  placeholder="Ask me: 'recipe with elubo', 'perfect tech phone under 400k', etc."
                  value={copilotInput}
                  onChange={(e) => setCopilotInput(e.target.value)}
                  className="flex-1 bg-slate-900 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-amber-400 placeholder-gray-500 font-medium"
                />
                <button
                  type="submit"
                  className="px-4 bg-gradient-to-r from-amber-500 to-amber-600 hover:brightness-110 font-bold text-slate-950 text-xs rounded-xl uppercase tracking-wider shadow-md active:scale-95 cursor-pointer flex items-center justify-center animate-duration-500"
                >
                  Ask AI
                </button>
              </form>
