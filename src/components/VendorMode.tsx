...
export default function VendorMode({
  ...
  const handleAIOptimize = async () => {
    ...
    try {
      const resp = await fetch('/api/gemini/vendor-optimize', {
        method: 'POST',
        headers: {
  ...
  // Earnings are estimated as delivered orders + dummy historical completed sums
  const completedOrders = orders.filter(o => o.status === 'Delivered');
  const deliveredRevenue = completedOrders.reduce((sum, o) => sum + o.totalAmount, 0);

  // Default hypothetical onboarding revenue for visual premium touch
  const basePreLiveRevenue = 412500;
  const totalRevenue = basePreLiveRevenue + deliveredRevenue;
  ...
  return (
    <div className="h-full flex flex-col justify-between bg-slate-50 dark:bg-slate-900 overflow-hidden relative text-left">
      ...
      <header className="h-14 bg-amber-500 text-slate-950 px-4 flex items-center justify-between border-b border-amber-600 z-10 shrink-0">
        ...
        {/* Quick Swite back to Buyer Mode right from Header! */}
        <button
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="col-span-2 p-4 bg-gradient-to-br from-slate-900 to-slate-950 rounded-2xl text-white space-y-1.5 shadow-sm relative overflow-hidden border border-slate-800">
                  ...
                  <span className="text-[10px] font-extrabold text-amber-400 uppercase tracking-widest block">
                    Available Payout Balance (Secure escrow)
                  </span>
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-2xl font-black">{formatNaira(totalRevenue)}</span>
                    <span className="text-[10px] text-emerald-400 font-bold">● Active 100%</span>
                  </div>
                  <p className="text-[9.5px] text-slate-400 leading-normal">
                    Fulfilling pending buy-orders routes funds instantly through integrated local accounts to your bank.
                  </p>
                </div>
              </div>
          ...
          {vendorTab === 'earnings' && (
            ...
            >
              ...
              <div className="p-4 bg-slate-950 text-white rounded-2xl space-y-1.5 shadow-sm">
                <span className="text-[9px] text-gray-400 font-extrabold uppercase tracking-wide">Gross Wallet Volume</span>
                <p className="text-2xl font-black text-amber-400">{formatNaira(totalRevenue)}</p>
                <p className="text-[10px] text-gray-400">Includes historical balance and released payment gateway allocations.</p>
              </div>
