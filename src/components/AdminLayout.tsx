...
import { 
  ...
  ChevronLeft, Menu, Truck, Compass, Star, ShieldAlert, X, Save, Headphones, ShieldCheck, Banknote
} from 'lucide-react';
...
interface AdminLayoutProps {
  ...
  onUpdateAdminSession: (session: Partial<{ id: string; name: string; level: string; email?: string; phone?: string; avatar?: string; totpEnabled?: boolean }>) => void;
...
export default function AdminLayout({
  ...
  // Sidebar navigation is desktop-only (see the `hidden md:flex` aside
  // below); on phone-width screens this drawer is the only way to switch
  // between admin sections, opened from the hamburger button in the header.
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const [settingsData, reportsData, teamData, withdrawalsData] = await Promise.all([
          api.getSettings(),
          api.getReports(),
          api.getAdminTeam(),
  ...
  const handleAdminAvatarSelected = async (file: File | undefined) => {
    if (!file) return;
    setUploadingAdminAvatar(true);
    try {
      const { avatar } = await api.uploadMyAdminAvatar(file);
      onUpdateAdminSession({ avatar });
      handleToast('Profile picture updated.', 'success');
    } catch (err: any) {
      handleToast(err.message || 'Could not upload image.', 'error');
    } finally {
  ...
  const handleAdminChangePassword = async () => {
    if (adminNewPw.length < 6) return handleToast('New password must be at least 6 characters.', 'error');
    setSavingAdminPw(true);
    try {
      await api.changeMyAdminPassword(adminCurrentPw, adminNewPw);
      setAdminCurrentPw('');
      setAdminNewPw('');
      handleToast('Password changed.', 'success');
    } catch (err: any) {
      handleToast(err.message || 'Could not change password.', 'error');
    } finally {
  ...
  // 2FA
  const [adminTotpEnabled, setAdminTotpEnabled] = useState<boolean>(!!adminSession?.totpEnabled);
  const [adminSetupData, setAdminSetupData] = useState<{ secret: string; qrCodeDataUrl: string } | null>(null);
  const [adminTotpSetupCode, setAdminTotpSetupCode] = useState('');
  const [admin2faBusy, setAdmin2faBusy] = useState(false);
  const [showAdminDisable2FA, setShowAdminDisable2FA] = useState(false);
  const [adminDisablePw, setAdminDisablePw] = useState('');

  const handleAdminStart2FASetup = async () => {
    setAdmin2faBusy(true);
    try {
      const data = await api.setupAdmin2FA();
      setAdminSetupData(data);
    } catch (err: any) {
      handleToast(err.message || 'Could not start 2FA setup.', 'error');
    } finally {
      setAdmin2faBusy(false);
    }
  };

  const handleAdminConfirm2FA = async () => {
    setAdmin2faBusy(true);
    try {
      await api.enableAdmin2FA(adminTotpSetupCode.trim());
      setAdminTotpEnabled(true);
      setAdminSetupData(null);
      setAdminTotpSetupCode('');
      onUpdateAdminSession({ totpEnabled: true });
      handleToast('Two-factor authentication enabled.', 'success');
    } catch (err: any) {
      handleToast(err.message || 'Incorrect code.', 'error');
    } finally {
      setAdmin2faBusy(false);
    }
  };
  ...
  const handleAdminDisable2FA = async () => {
    setAdmin2faBusy(true);
    try {
      await api.disableAdmin2FA(adminDisablePw);
      setAdminTotpEnabled(false);
      setShowAdminDisable2FA(false);
      setAdminDisablePw('');
      onUpdateAdminSession({ totpEnabled: false });
      handleToast('Two-factor authentication turned off.', 'success');
    } catch (err: any) {
      handleToast(err.message || 'Incorrect password.', 'error');
    } finally {
      setAdmin2faBusy(false);
    }
  };
  ...
  const handleUpdateWithdrawalStatus = async (id: string, status: 'Approved' | 'Rejected' | 'Paid', notes?: string) => {
    try {
      const updated = await api.updateWithdrawalStatus(id, status, notes);
      setWithdrawals((prev) => prev.map((w) => (w.id === id ? updated : w)));
      handleToast(`Withdrawal ${status.toLowerCase()}.`, 'success');
    } catch (err: any) {
      handleToast(err.message || 'Could not update withdrawal status.', 'error');
    }
  };

  const handleUpdateVendorCommission = async (vendorId: string, commissionPercent: number | null) => {
    try {
      await api.updateVendorCommission(vendorId, commissionPercent);
      handleToast('Vendor commission rate updated.', 'success');
    } catch (err: any) {
      handleToast(err.message || 'Could not update commission rate.', 'error');
    }
  };

  const handleUpdateProductCommission = async (productId: string, commissionPercent: number | null) => {
    try {
      await api.updateProductCommission(productId, commissionPercent);
      handleToast('Product commission rate updated.', 'success');
    } catch (err: any) {
      handleToast(err.message || 'Could not update commission rate.', 'error');
    }
  };
  ...
  const sidebarItems = [
    ...
    { id: 'withdrawals', label: 'Vendor Withdrawals', icon: Banknote },
    { id: 'settings', label: 'System Settings', icon: Settings },
    { id: 'support', label: 'Support Live Chat', icon: Headphones },
  ] as const;

  // Which sections each admin role/level can see. "Admin" and "Superuser
  // Level-4" are treated as full access. Roles not listed here fall back
  // to full access too, so inviting an admin with a custom/typo'd level
  // never accidentally locks them out of everything.
  const ROLE_TAB_ACCESS: Record<string, readonly AdminTab[] | 'all'> = {
    'Admin': 'all',
    'Superuser Level-4': 'all',
    'Manager': ['dashboard', 'vendors', 'kyc', 'reports', 'products', 'orders', 'users', 'support'],
    'Office Assistant': ['dashboard', 'vendors', 'kyc', 'products', 'orders', 'support'],
    'Support Agent': ['dashboard', 'orders', 'reports', 'support'],
  };

  const currentAccess = ROLE_TAB_ACCESS[adminSession?.level || ''] ?? 'all';
  const visibleSidebarItems = currentAccess === 'all'
    ? sidebarItems
    : sidebarItems.filter((item) => (currentAccess as readonly string[]).includes(item.id));

  useEffect(() => {
    if (currentAccess !== 'all' && !(currentAccess as readonly string[]).includes(activeTab)) {
  ...
  return (
    <div className={`flex w-full min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 font-sans transition-colors duration-300 relative`}>
      ...
      {/* MOBILE NAV DRAWER — the sidebar above is desktop-only (`hidden
          md:flex`), so this is what phone-width screens use instead to
          reach every admin section. Opened from the hamburger button in
          the mobile header. */}
      <AnimatePresence>
        {isMobileNavOpen && (
          <>
            ...
            <motion.aside
              ...
              style={{ backgroundColor: '#003D36' }}
              className="md:hidden fixed inset-y-0 left-0 w-72 max-w-[80vw] z-50 flex flex-col text-white shadow-2xl"
            >
              <div className="p-5 flex items-center justify-between border-b border-[#95C93D]/10 shrink-0">
                <div className="flex items-center gap-1.5">
                  <Logo size="md" />
                  <span className="text-[10px] font-black tracking-widest text-[#95C93D] uppercase font-mono bg-[#95C93D]/10 px-2 py-0.5 rounded-md border border-[#95C93D]/20">ADMIN</span>
                </div>
                <button
                  onClick={() => setIsMobileNavOpen(false)}
                ...
                {visibleSidebarItems.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 py-3 px-4 text-xs font-bold rounded-xl transition-all cursor-pointer ${
                        isActive
                          ? 'bg-[#95C93D] text-slate-950 shadow-md font-black'
                          : 'text-gray-300 hover:bg-white/5 hover:text-white'
                      }`}
                    >
                      <Icon className={`w-4.5 h-4.5 shrink-0 ${isActive ? 'text-slate-950' : 'text-gray-400'}`} />
                      <span>{item.label}</span>
                    </button>
                  );
                })}
              ...
              <div className="p-4 border-t border-[#95C93D]/20 space-y-2 bg-[#002f2a] shrink-0">
                <button
                  onClick={onBackToApp}
                  className="w-full py-2.5 px-4 bg-[#95C93D]/10 hover:bg-[#95C93D]/20 text-[#95C93D] border border-[#95C93D]/30 text-xs font-black rounded-xl cursor-pointer flex items-center justify-center gap-2"
                >
                  <ArrowLeft className="w-4 h-4" />
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div className="flex md:hidden items-center gap-2">
              <button
                ...
                className="p-2 -ml-1 rounded-lg text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-slate-800"
                      ...
                      return (
                        <>
                          ...
                          <div className="p-2.5 border-t border-gray-100 dark:border-slate-800 bg-slate-50/20 shrink-0 flex flex-wrap gap-2 items-center">
                            ...
                            {[
                              { label: 'Payment status', text: "Hello! We've checked your order — your payment is confirmed and will be released to the vendor once DELIVERI marks it delivered." },
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="relative">
                  {adminSession?.avatar ? (
                    <img src={adminSession.avatar} alt="Profile" className="w-14 h-14 rounded-full object-cover border border-gray-200 dark:border-gray-800" />
                  ) : (
                    <div className="w-14 h-14 rounded-full bg-[#95C93D]/10 text-[#95C93D] flex items-center justify-center text-lg font-black">
                  ...
                  <button
                    ...
                    className="absolute -bottom-1 -right-1 w-5 h-5 bg-[#95C93D] text-[#003D36] rounded-full flex items-center justify-center cursor-pointer shadow-md"
                  >
                  ...
                  <input
                    ...
                    className="hidden"
                    onChange={(e) => handleAdminAvatarSelected(e.target.files?.[0])}
                  />
                </div>
                <span className="text-[10px] text-gray-400">{uploadingAdminAvatar ? 'Uploading...' : 'Click the icon to change your photo'}</span>
              </div>
              ...
              <form onSubmit={handleSaveProfile} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3.5">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-gray-400 block uppercase font-mono">Email</label>
                    <p className="w-full bg-slate-100 dark:bg-slate-800/60 border border-gray-205 dark:border-slate-800 rounded-xl px-3.5 py-2 text-xs text-gray-500 dark:text-gray-400 font-mono truncate">
                      {profileEmail}
                    </p>
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-gray-400 block uppercase font-mono">Access Level</label>
                    <p className="w-full bg-slate-100 dark:bg-slate-800/60 border border-gray-205 dark:border-slate-800 rounded-xl px-3.5 py-2 text-xs text-gray-500 dark:text-gray-400 font-bold">
                      {profileLevel}
                    </p>
                  </div>
                </div>
              ...
              {/* Password change */}
              <div className="mt-5 pt-5 border-t border-gray-100 dark:border-slate-800 space-y-2.5">
                <h4 className="text-[10px] font-black uppercase text-gray-400 font-mono">Change Password</h4>
                <input type="password" value={adminCurrentPw} onChange={(e) => setAdminCurrentPw(e.target.value)} placeholder="Current password" className="w-full bg-slate-50 dark:bg-slate-950 border border-gray-205 dark:border-slate-800 rounded-xl px-3.5 py-2 text-xs focus:ring-1 focus:ring-[#95C93D] focus:outline-none text-gray-901 dark:text-white" />
                <input type="password" value={adminNewPw} onChange={(e) => setAdminNewPw(e.target.value)} placeholder="New password" className="w-full bg-slate-50 dark:bg-slate-950 border border-gray-205 dark:border-slate-800 rounded-xl px-3.5 py-2 text-xs focus:ring-1 focus:ring-[#95C93D] focus:outline-none text-gray-901 dark:text-white" />
                <button
                  type="button"
                  onClick={handleAdminChangePassword}
                  disabled={savingAdminPw || !adminCurrentPw || !adminNewPw}
                  className="w-full py-2 bg-slate-900 dark:bg-white dark:text-gray-900 text-white text-xs font-black rounded-xl cursor-pointer disabled:opacity-50"
                >
                  {savingAdminPw ? 'Saving...' : 'Update Password'}
                </button>
              </div>

              {/* 2FA */}
              <div className="mt-5 pt-5 border-t border-gray-100 dark:border-slate-800 space-y-2.5">
                <h4 className="text-[10px] font-black uppercase text-gray-400 font-mono flex items-center gap-1.5">
                  <ShieldCheck className="w-3.5 h-3.5 text-[#95C93D]" /> Two-Factor Authentication
                </h4>
                {adminTotpEnabled ? (
                  showAdminDisable2FA ? (
                    <div className="space-y-2">
                      <input type="password" value={adminDisablePw} onChange={(e) => setAdminDisablePw(e.target.value)} placeholder="Confirm your password" className="w-full bg-slate-50 dark:bg-slate-950 border border-gray-205 dark:border-slate-800 rounded-xl px-3.5 py-2 text-xs focus:ring-1 focus:ring-[#95C93D] focus:outline-none text-gray-901 dark:text-white" />
                      <div className="flex gap-2">
                        <button type="button" onClick={handleAdminDisable2FA} disabled={admin2faBusy || !adminDisablePw} className="flex-1 py-2 bg-red-600 hover:bg-red-500 disabled:opacity-50 text-white rounded-xl text-xs font-black cursor-pointer">
                          {admin2faBusy ? 'Please wait...' : 'Turn Off 2FA'}
                        </button>
                        <button type="button" onClick={() => setShowAdminDisable2FA(false)} className="px-3 py-2 bg-gray-100 dark:bg-slate-800 rounded-xl text-xs font-bold cursor-pointer">Cancel</button>
                      </div>
                    </div>
                  ) : (
                    <div className="flex items-center justify-between">
                      <span className="flex items-center gap-1.5 text-emerald-500 text-xs font-bold"><Save className="w-3.5 h-3.5" /> Enabled</span>
                      <button type="button" onClick={() => setShowAdminDisable2FA(true)} className="text-[11px] text-red-500 font-bold cursor-pointer">Turn off</button>
                    </div>
                  )
                ) : adminSetupData ? (
                  <div className="space-y-2.5">
                    <p className="text-[10px] text-gray-400">Scan with Google Authenticator, then enter the code.</p>
                    <img src={adminSetupData.qrCodeDataUrl} alt="2FA QR code" className="w-32 h-32 mx-auto rounded-xl border border-gray-200 dark:border-gray-800" />
                    <input
                      type="text"
                      inputMode="numeric"
                      maxLength={6}
                      value={adminTotpSetupCode}
                      onChange={(e) => setAdminTotpSetupCode(e.target.value.replace(/\D/g, ''))}
                      placeholder="000000"
                      className="w-full text-center tracking-[0.3em] bg-slate-50 dark:bg-slate-950 border border-gray-205 dark:border-slate-800 rounded-xl px-3.5 py-2 text-xs focus:ring-1 focus:ring-[#95C93D] focus:outline-none text-gray-901 dark:text-white font-mono"
                    />
                    <div className="flex gap-2">
                      <button type="button" onClick={handleAdminConfirm2FA} disabled={admin2faBusy || adminTotpSetupCode.length !== 6} className="flex-1 py-2 bg-[#95C93D] text-[#003D36] rounded-xl text-xs font-black cursor-pointer disabled:opacity-50">
                        {admin2faBusy ? 'Verifying...' : 'Confirm & Enable'}
                      </button>
                      <button type="button" onClick={() => setAdminSetupData(null)} className="px-3 py-2 bg-gray-100 dark:bg-slate-800 rounded-xl text-xs font-bold cursor-pointer">Cancel</button>
                    </div>
                  </div>
                ) : (
                  <button
                    type="button"
                    onClick={handleAdminStart2FASetup}
                    disabled={admin2faBusy}
                    className="px-3 py-2 bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 text-xs font-bold rounded-xl cursor-pointer"
                  >
                    {admin2faBusy ? 'Loading...' : 'Set Up 2FA'}
                  </button>
                )}
              </div>
            </motion.div>
