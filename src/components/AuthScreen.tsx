...
import { Mail, Lock, User, Phone, Check, ShieldCheck, Loader2, Chrome, AlertCircle } from 'lucide-react';
...
export default function AuthScreen({ onAuthCompleted, onSkip, initialMode = 'login', adminUsers, onAdminSelected }: AuthScreenProps) {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-slate-950 font-sans justify-between overflow-hidden relative">
      ...
      {/* Header section with skipped state */}
      <div className="flex items-center justify-between p-5 z-20 shrink-0">
        ...
        {/* Skipping deactivated according to Requirement 5 */}
        <span className="text-[10px] uppercase font-black tracking-widest text-[#95C93D] font-mono animate-pulse bg-[#003D36] px-2.5 py-1 rounded border border-[#95C93D]/30">
          ● Secure Portal Gate
        </span>
      </div>
      ...
      <div className="flex-1 px-6 flex flex-col justify-center overflow-y-auto z-10 py-2">
        ...
        <div className="text-center mb-6 shrink-0">
          <h2 className="text-xl font-black text-white tracking-tight">
            {tab === 'login' ? 'Welcome Back!' : 'Create Nigeria No.1 Storefront'}
          </h2>
          <p className="text-[10px] text-gray-400 mt-1 max-w-xs mx-auto">
            TradeEase bridges dynamic logistics with local instant payments and secure order placement.
          </p>
        </div>
        ...
        {success && (
          <div className="p-4 bg-emerald-950/50 border border-emerald-500/20 rounded-xl text-center space-y-1.5 mb-4 animate-pulse">
            ...
            <h3 className="text-xs font-bold text-emerald-400">Successfully Securely Logged In</h3>
            <p className="text-[9px] text-emerald-350">Routing to Nigeria's trade zone...</p>
          </div>
        )}
        ...
        <form onSubmit={handleTraditionalSubmit} className="space-y-3.5">
          ...
          {/* Account Route Selection - Drop-down according to Correction Item 1 */}
          <div className="space-y-1 bg-slate-900/60 p-2.5 rounded-xl border border-white/5">
            ...
            >
              <option value="buyer">🛍️ Buyer Account (Browse, Purchase, Escrow Wallet)</option>
              <option value="vendor">🏪 Vendor Merchant Account (Add Products, Earnings Wallet)</option>
            </select>
          </div>
          {tab === 'signup' && (
            <div className="space-y-1">
              <label className="text-[9.5px] font-black uppercase text-gray-400 tracking-wider pl-1 font-mono">Full Name / Vendor Store Name</label>
              <div className="relative">
                <User className="absolute left-3.5 top-3 w-4 h-4 text-gray-500" />
                <input
                  type="text"
                  placeholder="e.g. Fauch Agency Ltd, Kunle Adeleke"
                  value={name}
          ...
          <div className="space-y-1">
            <label className="text-[9.5px] font-black uppercase text-gray-400 tracking-wider pl-1 font-mono">Email Address</label>
            <div className="relative">
          ...
          {tab === 'signup' && (
            <div className="space-y-1">
              <label className="text-[9.5px] font-black uppercase text-gray-400 tracking-wider pl-1 font-mono">Mobile (WhatsApp Enabled Preferred)</label>
              <div className="relative">
          ...
          <div className="space-y-1">
            <label className="text-[9.5px] font-black uppercase text-gray-400 tracking-wider pl-1 font-mono">Secure Access Password</label>
            <div className="relative">
          ...
          >
            {loading ? (
              ...
              <span>{tab === 'login' ? `Sign In as ${accountRole.toUpperCase()}` : `Build Free ${accountRole.toUpperCase()} Profile`}</span>
            )}
          </button>
        </form>
        ...
        <div className="grid grid-cols-2 gap-2.5 shrink-0">
          ...
          >
            ...
            <span className="truncate">Google Access</span>
          </button>
          ...
          >
            ...
            <span className="truncate">Apple login</span>
          </button>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-white/5 bg-slate-900/30 flex flex-col items-center gap-2 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[9.5px] font-bold text-gray-500">Multi-Channel Escrow Shield Protection</span>
        </div>
        
        {/* Launch Corporate portal differently as requested by Item 2 / Item 12 */}
        {onAdminSelected && (
          <button
            type="button"
            onClick={onAdminSelected}
            className="text-[10px] font-extrabold text-amber-500/80 hover:text-amber-400 transition-colors cursor-pointer mt-0.5 hover:underline underline-offset-4 flex items-center gap-1 bg-white/5 py-1 px-3 rounded-lg border border-white/5"
          >
            <span>🏢 Access Corporate Admin Terminal</span>
          </button>
        )}
      </div>
          ...
          >
            ...
            >
              ...
              <div className="text-center">
                ...
                <h4 className="text-xs font-black text-white">Choose a Facebook Identity</h4>
                <p className="text-[9px] text-gray-400">to securely bind your TradeEase storefront</p>
              </div>
