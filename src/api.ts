...
// Upload up to 10 product photos at once. Files are appended to whatever
// images the product already has (capped at 10 total server-side).
export async function uploadProductImages(productId: string, files: File[]): Promise<Product> {
...
export const removeProductImage = (productId: string, url: string) =>
  request<Product>(`/products/${productId}/images`, { method: 'DELETE', body: JSON.stringify({ url }) });

// Uploads the actual ebook file(s) for a digital product. Pass either or
// both — a vendor can offer just one format, or let buyers choose.
export async function uploadProductEbookFiles(productId: string, files: { epub?: File; pdf?: File }): Promise<Product> {
  const form = new FormData();
  if (files.epub) form.append('epub', files.epub);
  if (files.pdf) form.append('pdf', files.pdf);
  const token = getToken();
  const res = await fetch(`${API_BASE}/api/products/${productId}/ebook`, {
    method: 'POST',
    headers: token ? { Authorization: `Bearer ${token}` } : undefined,
    body: form,
  });
...
// Downloads a purchased ebook in the buyer's chosen format. Triggers a
// browser download rather than returning a URL, since the endpoint is
// authenticated (requires proof of purchase) and can't just be linked to
// directly.
export async function downloadEbookFile(productId: string, format: 'epub' | 'pdf', fileName: string): Promise<void> {
  const token = getToken();
  const res = await fetch(`${API_BASE}/api/products/${productId}/ebook/${format}`, {
    headers: token ? { Authorization: `Bearer ${token}` } : undefined,
  });
  if (!res.ok) {
...
export const getOrders = (vendorId?: string, useAdminToken = false) => request<Order[]>(`/orders${vendorId ? `?vendorId=${vendorId}` : ''}`, {}, useAdminToken);
export const getOrderQuote = (quote: { items: Array<{ productId: string; quantity: number }>; state: string; logisticsProviderId: string; lastMileKm: number; deliveryUrgency: 'economy' | 'speedy' | 'flash' }) =>
  request<{ subtotal: number; shippingCost: number; total: number; items: Array<{ productId: string; title: string; unitPrice: number; quantity: number; lineTotal: number }>; logisticsProviderId: string; logisticsProviderName: string; totalWeightKg: number; totalDistanceKm: number; expiresAt: string }>('/orders/quote', { method: 'POST', body: JSON.stringify(quote) });

export const placeOrder = (order: Partial<Order> & { items: Array<{ productId: string; productTitle?: string; price?: number; quantity: number; image?: string }>; lastMileKm?: number; deliveryUrgency?: 'economy' | 'speedy' | 'flash' }) =>
...
export const getSupportChats = (useAdminToken = false) => request<any[]>('/support-chats', {}, useAdminToken);
export const getMySupportChats = (email?: string) => request<any[]>(`/support-chats/mine${email ? `?email=${encodeURIComponent(email)}` : ''}`);
...
export const addSupportMessage = (chatId: string, sender: 'user' | 'admin', senderName: string, content: string, userEmail?: string) =>
  request<any>(`/support-chats/${chatId}/messages`, { method: 'POST', body: JSON.stringify({ sender, senderName, content, userEmail }) });
...
export const updateVendorState = (vendorId: string, state: { accountStatus?: 'Active'|'Suspended'|'Under Review'; storeStatus?: 'Pending'|'Active'|'Suspended'|'Rejected'; payoutStatus?: 'Active'|'On Hold'|'Frozen'; suspensionReason?: string | null }) =>
  request<AdminVendor>(`/vendors/${vendorId}/state`, { method: 'PUT', body: JSON.stringify(state) }, true);
...
export const deleteNotification = (id: string) => request<{success:boolean}>(`/notifications/${id}`, { method:'DELETE' });

// ---------- Analytics ----------
export const getAdminAnalytics = () => request<any>('/analytics/admin', {}, true);
export const getVendorAnalytics = () => request<any>('/analytics/vendor');

// ---------- Refunds / disputes ----------
export const getRefunds = () => request<any[]>('/refunds', {}, true);
export const getMyRefunds = () => request<any[]>('/refunds/mine');
export const requestRefund = (orderId:string, reason:string, amount?:number) => request<any>('/refunds',{method:'POST',body:JSON.stringify({orderId,reason,amount})});
export const updateRefundStatus = (id:string,status:string,approvedAmount?:number,notes?:string) => request<any>(`/refunds/${id}/status`,{method:'PUT',body:JSON.stringify({status,approvedAmount,notes})},true);

// ---------- Operations / AI ----------
export const getRuntimeStatus = () => request<any>('/ops/runtime',{},true);
export const getAdminAiInsights = (days=30) => request<any>('/gemini/admin-insights',{method:'POST',body:JSON.stringify({days})},true);
