...
export default function App() {
  ...
  const [authPreferredRole, setAuthPreferredRole] = useState<'buyer' | 'vendor'>('buyer');
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      // Products are public marketplace data and must load for guests.
      // Orders are protected and therefore must only be requested when a
      // user is authenticated. Keeping these requests independent prevents
      // a guest's expected 401 from blocking the public product catalogue.
      const productsPromise = api.getProducts();
      const ordersPromise = currentUser ? api.getOrders() : Promise.resolve([] as Order[]);
      const chatsPromise = api.getMySupportChats(currentUser?.email);

      const [productsResult, ordersResult, chatsResult] = await Promise.allSettled([
        productsPromise,
        ordersPromise,
        chatsPromise,
      ]);
      ...
      if (productsResult.status === 'fulfilled') {
        ...
        console.error('Error loading public products from API:', productsResult.reason);
      }

      if (ordersResult.status === 'fulfilled') {
        setOrdersList(ordersResult.value);
      } else {
        console.error('Error loading orders from API:', ordersResult.reason);
      }

      if (chatsResult.status === 'fulfilled') {
        setSupportChats(
          [...chatsResult.value].sort((a, b) => new Date(b.lastMessageAt).getTime() - new Date(a.lastMessageAt).getTime())
        );
      } else {
        console.error('Error loading support chats from API:', chatsResult.reason);
      }
    };
  ...
  const handleAddSupportMessage = async (sessionId: string, sender: 'user' | 'admin', senderName: string, content: string) => {
    ...
    const userEmail = supportChats.find((c) => c.id === sessionId)?.userEmail || currentUser?.email || 'guest@tradease.ng';
    ...
    try {
      const updatedChat = await api.addSupportMessage(sessionId, sender, senderName, content, userEmail);
  ...
  const handlePlaceNewOrder = async (shippingDetails: { 
    ...
    const createdOrder = await api.placeOrder({
      ...
      lastMileKm: shippingDetails.lastMileKm,
      deliveryUrgency: shippingDetails.deliveryUrgency,
    } as any);
  ...
  const handleAddCustomProduct = async (newProduct: Product): Promise<Product> => {
        ...
        >
          <AnimatePresence mode="wait">
            ...
            {appState === 'auth' && (
              ...
              >
                <AuthScreen
                  initialAccountRole={authPreferredRole}
                    ...
                    {activeRole === 'buyer' ? (
                      ...
                      >
                        <BuyerMode
                          ...
                          currentUser={currentUser}
                          onUserUpdated={(updates) => setCurrentUser((prev) => (prev ? { ...prev, ...updates } : prev))}
                          onLogout={() => { api.logoutAccount(); setCurrentUser(null); }}
                          onTriggerLogin={(preferredRole) => {
                            setAuthPreferredRole(preferredRole || 'buyer');
                            setAppState('auth');
                          }}
                        />
                      </motion.div>
