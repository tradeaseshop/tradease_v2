...
async function startServer() {
  ...
  try {
    ...
    // Optional, explicitly enabled one-time recovery for an existing production admin.
    // This runs after first-boot seeding so it also works on a brand-new database.
    const { runAdminPasswordReset } = await import('./backend/adminPasswordReset');
    runAdminPasswordReset();
  } catch (err) {
