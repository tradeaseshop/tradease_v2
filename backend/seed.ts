...
function seedCategories() {
  const insert = db.prepare(
    'INSERT OR IGNORE INTO categories (id, name, icon_name, color, display_order) VALUES (?, ?, ?, ?, ?)'
  );
  ...
  let order = 0;
  const legacyAliases = [
    { id: 'electronics', name: 'Electronics (Legacy)', iconName: 'Smartphone', color: '' },
    { id: 'fashion', name: 'Fashion (Legacy)', iconName: 'Shirt', color: '' },
    { id: 'ebooks', name: 'E-Books (Legacy)', iconName: 'BookOpen', color: '' },
    { id: 'food', name: 'Food & Groceries (Legacy)', iconName: 'Utensils', color: '' },
    { id: 'home', name: 'Home & Kitchen (Legacy)', iconName: 'Home', color: '' },
    { id: 'beauty', name: 'Beauty & Style (Legacy)', iconName: 'Sparkles', color: '' },
  ];
  for (const c of [...TRADEASE_CATALOG, ...legacyAliases, ...suremartCategories]) {
    insert.run(c.id, c.name, c.iconName, c.color, order++);
  }
}

function seedVendorsAndAdmins() {
  const adminPassword = process.env.ADMIN_INITIAL_PASSWORD || (IS_PRODUCTION ? '' : 'dev-only-change-me');
  if (!adminPassword) throw new Error('ADMIN_INITIAL_PASSWORD must be configured before production seeding.');
  db.prepare('INSERT INTO admins (id, name, email, password_hash, level, phone) VALUES (?, ?, ?, ?, ?, ?)').run(
    randomUUID(), 'Chief Architect', 'admin@tradease.ng', hashPassword(adminPassword), 'Superuser Level-4', '+234 1 0000 8888'
  );
...
function seedLogisticsProviders() {
  ...
  for (const p of DEFAULT_LOGISTICS_PROVIDERS as any[]) {
    insert.run(
      p.id, p.name, p.code, p.type, p.status, p.apiEndpoint, process.env[`LOGISTICS_${p.code.toUpperCase()}_API_KEY`] || null, p.webhookUrl, process.env[`LOGISTICS_${p.code.toUpperCase()}_WEBHOOK_SECRET`] || null,
...
export function runSeed(options: { productionBootstrap?: boolean } = {}) {
  const productionBootstrap = options.productionBootstrap === true;
  if (IS_PRODUCTION && !productionBootstrap && process.env.ALLOW_DEMO_SEED !== 'true') {
    throw new Error('Refusing to reset a production database. Set ALLOW_DEMO_SEED=true only for an intentional disposable demo environment.');
  }
  ...
  seedDeliveryZones();
  console.log('Done.\n');
  console.log('Admin account seeded. Credentials come from ADMIN_INITIAL_PASSWORD.');
  if (!IS_PRODUCTION) console.log('Development buyer demo: onyekachi@mail.com / password123');
}
