...
export function toProduct(row: any) {
  return {
    ...
    // null = uses the vendor's rate (or the platform default if the vendor
    // has no override either).
...
export function toOrder(row: any, items: any[]) {
  return {
    ...
    items: items.map((i) => ({
      ...
      quantity: i.quantity,
      image: i.image,
      digitalFormat: i.digital_format || undefined,
    })),
    ...
    serverSubtotal: row.server_subtotal,
    quoteShippingCost: row.quote_shipping_cost,
    vendorOrders: row.vendor_orders || [],
  };
}
...
export function toAdminVendor(row: any) {
  return {
    ...
    // null = uses the platform's global commission rate instead of a
    // custom one for this vendor.
    commissionPercent: row.commission_percent,
...
export function toWithdrawal(row: any) {
  return {
    id: row.id,
    vendorId: row.vendor_id,
    vendorName: row.vendor_name,
