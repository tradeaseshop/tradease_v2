...
router.put('/vendors/:id/status', requireRole('admin'), (req, res) => {
  ...
  db.prepare("UPDATE vendors SET status=?, store_status=?, account_status=CASE WHEN ?='Approved' THEN 'Active' WHEN ?='Rejected' THEN 'Under Review' ELSE account_status END WHERE id=?").run(status, status==='Approved'?'Active':status==='Rejected'?'Rejected':'Pending', status, status, req.params.id);
...
router.put('/vendors/:id/state', requireRole('admin'), (req, res) => {
  const { accountStatus, storeStatus, payoutStatus, suspensionReason } = req.body || {};
  const allowedAccount = ['Active','Suspended','Under Review'];
  const allowedStore = ['Pending','Active','Suspended','Rejected'];
  const allowedPayout = ['Active','On Hold','Frozen'];
  if (accountStatus !== undefined && !allowedAccount.includes(accountStatus)) return res.status(400).json({ error:'Invalid accountStatus' });
  if (storeStatus !== undefined && !allowedStore.includes(storeStatus)) return res.status(400).json({ error:'Invalid storeStatus' });
  if (payoutStatus !== undefined && !allowedPayout.includes(payoutStatus)) return res.status(400).json({ error:'Invalid payoutStatus' });
  const existing = db.prepare('SELECT * FROM vendors WHERE id=?').get(req.params.id) as any;
  if (!existing) return res.status(404).json({ error:'Vendor not found' });
  db.prepare(`UPDATE vendors SET account_status=?, store_status=?, payout_status=?, suspension_reason=? WHERE id=?`).run(accountStatus ?? existing.account_status, storeStatus ?? existing.store_status, payoutStatus ?? existing.payout_status, suspensionReason ?? existing.suspension_reason, req.params.id);
  const row = db.prepare('SELECT * FROM vendors WHERE id=?').get(req.params.id) as any;
  audit(req,'vendor.state.update','vendor',req.params.id,{accountStatus,storeStatus,payoutStatus});
  return res.json(toAdminVendor(row));
});
...
router.put('/vendors/:id/commission', requireRole('admin'), (req, res) => {
  ...
  const row = db.prepare('SELECT * FROM vendors WHERE id = ?').get(req.params.id) as any;
  audit(req,'vendor.commission.update','vendor',req.params.id,{commissionPercent});
  res.json(toAdminVendor(row));
});
